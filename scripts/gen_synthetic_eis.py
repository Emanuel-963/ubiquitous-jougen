...
::
    ...
    python scripts/gen_synthetic_eis.py                 # 20 arquivos × 37 circuitos
...
Circuitos do registry adicionados ao gerador (4)
------------------------------------------------
REG-12  Porous-Coating-TLM      — Rs − (Rcoat‖Cpore) − (Rct‖CPEdl)
REG-13  MXene-Intercalation     — Rs − (Rsei‖CPEsei) − (Rct‖CPEdl) − Wfinite
REG-14  De-Levie-TLM            — Rs + √(Ri/Ydl)·coth(L·√(Ri·Ydl))
REG-15  Pseudo-Capacitance-CPE  — Rs − (Rct‖CPEdl) − (Rads‖Cads)
...
# ── Registry circuits added to synthetic generator ────────────────────────
# REG-12  Porous-Coating-TLM  [Rs − (Rcoat‖Cpore) − (Rct‖CPEdl)]
def _m_Porous_Coating_TLM(p, o):
    Rs, Rcoat, Cpore, Rct, Qdl, n_dl = p
    return Rs + _rc_par(o, Rcoat, Cpore) + _zarc(o, Rct, Qdl, n_dl)


# REG-13  MXene-Intercalation  [Rs − (Rsei‖CPEsei) − (Rct‖CPEdl) − Wfinite]
def _m_MXene_Intercalation(p, o):
    Rs, Rsei, Qsei, n_sei, Rct, Qdl, n_dl, AW, tau_d = p
    return (
        Rs
        + _zarc(o, Rsei, Qsei, n_sei)
        + _zarc(o, Rct, Qdl, n_dl)
        + _wfin_b(o, AW, tau_d)
    )


# REG-14  De-Levie-TLM  [Rs + √(Ri/Ydl)·coth(L·√(Ri·Ydl))]
def _m_De_Levie_TLM(p, o):
    Rs, Ri, Qdl, n_dl, L = p
    Ydl_per_L = Qdl * (1j * o) ** n_dl
    Zc = np.sqrt(Ri / (Ydl_per_L + 1e-30))
    gamma_L = L * np.sqrt(Ri * Ydl_per_L + 1e-30)
    with np.errstate(over="ignore", invalid="ignore"):
        coth_gL = np.where(
            np.abs(gamma_L.real) > 20,
            np.sign(gamma_L.real),
            1.0 / np.tanh(gamma_L + 1e-30),
        )
    return Rs + Zc * coth_gL


# REG-15  Pseudo-Capacitance-CPE  [Rs − (Rct‖CPEdl) − (Rads‖Cads)]
def _m_Pseudo_Capacitance(p, o):
    Rs, Rct, Qdl, n_dl, Rads, Cads = p
    return Rs + _zarc(o, Rct, Qdl, n_dl) + _rc_par(o, Rads, Cads)
...
_EXTENDED_MODELS: dict[str, Any] = {
    ...
    # Registry circuits
    "Porous-Coating-TLM": _m_Porous_Coating_TLM,
    "MXene-Intercalation": _m_MXene_Intercalation,
    "De-Levie-TLM": _m_De_Levie_TLM,
    "Pseudo-Capacitance-CPE": _m_Pseudo_Capacitance,
}
...
def simulate_spectrum(
    ...
    # Heteroscedastic noise: σ = noise_level·|component| + 5%·noise_level·mean|Z|
    # This models real instrument noise more accurately than plain magnitude scaling.
    Zmag_mean = float(np.abs(Z).mean())
    floor = noise_level * 0.05 * Zmag_mean
    sigma_r = noise_level * np.abs(Z.real) + floor
    sigma_i = noise_level * np.abs(Z.imag) + floor
    noise_r = rng.normal(0.0, sigma_r)
    noise_i = rng.normal(0.0, sigma_i)

    # Outliers: ~5 % dos pontos recebem ruído extra ×5 (simula artefatos)
    n_pts = len(Z)
    n_out = max(1, round(0.05 * n_pts))
    out_idx = rng.choice(n_pts, size=n_out, replace=False)
    noise_r[out_idx] += rng.normal(0.0, 5.0 * sigma_r[out_idx])
    noise_i[out_idx] += rng.normal(0.0, 5.0 * sigma_i[out_idx])
...
# Circuits with ≥ 9 parameters — generated at 2× the base rate so the
# training dataset is better balanced (complex topologies are harder to
# identify and benefit from more training examples).
_COMPLEX_CIRCUITS: frozenset[str] = frozenset(
    {
        "Three-ZARC",  # 10p
        "Rs-ZARC-ZARC-Wfinite",  # 9p
        "Rs-ZARC-ZARC-Wshort",  # 9p
        "Rs-ZARC-ZARC-Gerischer",  # 9p
        "Rs-ZARC-ZARC-CPE",  # 9p
        "Rs-ZARC-ZARC-TLM",  # 10p
        "Rs-ZARC-ZARC-ZARC-W",  # 11p
        "Rs-ZARC-ZARC-ZARC-Wfinite",  # 12p
        "Rs-ZARC-ZARC-ZARC-Gerischer",  # 12p
        "Rs-L-ZARC-ZARC-W",  # 9p
        "MXene-Intercalation",  # 9p
    }
)
...
def generate(
    ...
    noise_level: float,
    seed: int | None,
    complex_multiplier: int = 2,
) -> list[Path]:
    """Gera os arquivos e retorna lista de caminhos criados.

    Circuitos com ≥ 9 parâmetros recebem ``n_per_class * complex_multiplier``
    arquivos para compensar sua maior complexidade espectral e melhorar o
    balanceamento do dataset de treinamento do classificador ML.
    """
    ...
    for circuit_name in circuits:
        if circuit_name not in _MODELS:
            ...
            continue
        n = (
            n_per_class * complex_multiplier
            if circuit_name in _COMPLEX_CIRCUITS
            else n_per_class
        )
        ...
        log.info("  [%s] %d arquivos gerados", circuit_name, n)
